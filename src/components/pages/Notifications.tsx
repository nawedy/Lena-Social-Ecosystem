...
const Notifications: React.FC<NotificationsProps> = () => {
  ...
  const _agent = useMemo(() => new BskyAgent({ service: 'https://bsky.social' }), []);
  ...
  const _loadNotifications = useCallback(async () => {
    try {
      ...
      window.window.localStorage.setItem('lastNotificationCheck', new Date().toISOString());
    } catch (err: any) {
      ...
      <div className='max-w-2xl mx-auto'>
        ...
        <div className='space-y-4'>
          {notifications.map((notification) => (
            <div key={notification.cid} className='p-4 bg-white dark:bg-gray-800 rounded-lg shadow'>
              <div className='flex items-start'>
                ...
                <div className='ml-3 flex-1'>
                  <p className='text-sm text-gray-900 dark:text-white'>
                    ...
                    <span className='text-gray-500'>@{notification.author.handle}</span>
                  </p>
                  ...
                  <p className='mt-1 text-xs text-gray-500'>{formatTime(notification.indexedAt)}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          {notifications.length === 0 && (
            <div className='text-center text-gray-500 dark:text-gray-400'>No notifications yet</div>
          )}
        </div>
      </div>
    </div>
