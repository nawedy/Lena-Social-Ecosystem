...
class DatabaseMonitoringService {
  ...
  async collectDatabaseSizeMetrics() {
    ...
    `,
      [metrics.database_name, metrics.size_bytes, metrics.table_count, metrics.index_count]
    );
  }
  ...
  async recordQueryPerformance(queryText, executionTimeMs, rowsAffected, clientInfo = {}) {
    try {
      await db.query('SELECT record_query_performance($1, $2, $3, $4)', [
        queryText,
        executionTimeMs,
        rowsAffected,
  ...
  async recordError(error, queryText = null, clientInfo = {}) {
    try {
      ...
      `,
        [error.code, error.message, queryText, error.stack, JSON.stringify(clientInfo)]
      );
    } catch (insertError) {
