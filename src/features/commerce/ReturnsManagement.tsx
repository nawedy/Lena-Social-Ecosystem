...
import { ATProtocolOrderManagement, OrderRoute } from '../../services/atProtocolOrderManagement';
import { ATProtocolShipping, ShipmentLabel } from '../../services/atProtocolShipping';
...
interface ReturnRequest {
  ...
  status: 'pending' | 'approved' | 'shipped' | 'received' | 'refunded' | 'denied';
...
export const ReturnsManagement: React.FC = () => {
  ...
  return (
    <ScrollView className='flex-1 bg-gray-100 dark:bg-gray-900'>
      ...
      {/* Returns List */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Return Requests</Text>
        {returns
          .filter(
            (ret) =>
          ...
          .map((returnRequest) => (
            <TouchableOpacity
              ...
              onPress={() =>
                setSelectedReturn(selectedReturn?.uri === returnRequest.uri ? null : returnRequest)
              }
            ...
            >
              ...
              <Text className='text-gray-600 dark:text-gray-400'>
                Items: {returnRequest.items.reduce((sum, item) => sum + item.quantity, 0)}
              </Text>
              <Text className='text-gray-600 dark:text-gray-400'>
                Created: {format(new Date(returnRequest.createdAt), 'MMM d, yyyy')}
              </Text>
              ...
              {selectedReturn?.uri === returnRequest.uri && (
                <View className='mt-4'>
                  ...
                  <View className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4'>
                    <Text className='font-semibold mb-2 dark:text-white'>Return Reason</Text>
                    <Text className='text-gray-600 dark:text-gray-400'>{returnRequest.reason}</Text>

                    <Text className='font-semibold mt-4 mb-2 dark:text-white'>Items</Text>
                  ...
                  {returnRequest.status === 'pending' && (
                    <View className='flex-row flex-wrap'>
                      ...
                      >
                        <Text className='text-white text-center'>Approve Return</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        className='bg-red-500 rounded-lg p-2 m-1 flex-1'
                        onPress={() => denyReturn(returnRequest.uri, 'Not eligible for return')}
                      >
                        <Text className='text-white text-center'>Deny Return</Text>
                      </TouchableOpacity>
                    </View>
                  )}
                  ...
                  {returnRequest.status === 'received' && (
                    ...
                    >
                      <Text className='text-white text-center'>Process Refund</Text>
                    </TouchableOpacity>
                  )}
                </View>
              )}
      ...
      {/* Return Statistics */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Return Statistics</Text>
        <View className='flex-row flex-wrap'>
          <View className='w-1/2 p-2'>
            <Text className='text-gray-600 dark:text-gray-400'>Pending Returns</Text>
          ...
          <View className='w-1/2 p-2'>
            ...
            <Text className='text-2xl font-bold dark:text-white'>
              {returns.filter((r) => ['approved', 'shipped'].includes(r.status)).length}
            </Text>
          </View>
          <View className='w-1/2 p-2'>
