...
export const ContentAutomationDashboard: React.FC = () => {
  ...
  const [optimizations, setOptimizations] = useState<Record<string, ContentOptimization>>({});
  ...
  const _updateScheduleStatus = async (uri: string, status: ContentSchedule['status']) => {
    try {
  ...
  if (loading) {
    return (
      <View className='flex-1 justify-center items-center'>
        <Text className='text-lg dark:text-white'>Loading content automation...</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView className='flex-1 bg-gray-100 dark:bg-gray-900'>
      ...
      {/* Templates */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Content Templates</Text>
        {templates
          ...
          .map((template) => (
            <TouchableOpacity
              key={template.uri}
              onPress={() =>
                setSelectedTemplate(selectedTemplate === template.uri ? null : template.uri)
              }
              className='mb-4 border-b border-gray-200 pb-4'
            >
              <View className='flex-row justify-between'>
                <Text className='font-semibold dark:text-white'>{template.name}</Text>
              ...
              {selectedTemplate === template.uri && (
                <View className='mt-4'>
                  ...
                  <View className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4'>
                    <Text className='font-semibold mb-2 dark:text-white'>Template Content</Text>
                    <Text className='text-gray-600 dark:text-gray-400'>{template.template}</Text>

                    <Text className='font-semibold mt-4 mb-2 dark:text-white'>Variables</Text>
                  ...
                  {analytics[template.uri] && (
                    <View className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4'>
                      <Text className='font-semibold mb-2 dark:text-white'>Performance</Text>
                      <View className='flex-row flex-wrap'>
                        <View className='w-1/2 p-2'>
                          <Text className='text-gray-600 dark:text-gray-400'>Impressions</Text>
                          <Text className='text-2xl font-bold dark:text-white'>
                            {analytics[template.uri].metrics.impressions.toLocaleString()}
                          </Text>
                        </View>
                        <View className='w-1/2 p-2'>
                          <Text className='text-gray-600 dark:text-gray-400'>Engagements</Text>
                          <Text className='text-2xl font-bold dark:text-white'>
                            {analytics[template.uri].metrics.engagements.toLocaleString()}
                          </Text>
                        </View>
                        {analytics[template.uri].metrics.clicks !== undefined && (
                          <View className='w-1/2 p-2'>
                            <Text className='text-gray-600 dark:text-gray-400'>Clicks</Text>
                            <Text className='text-2xl font-bold dark:text-white'>
                              {analytics[template.uri].metrics.clicks?.toLocaleString()}
                            </Text>
                          </View>
                        )}
                        {analytics[template.uri].metrics.conversions !== undefined && (
                          <View className='w-1/2 p-2'>
                            <Text className='text-gray-600 dark:text-gray-400'>Conversions</Text>
                            <Text className='text-2xl font-bold dark:text-white'>
                              {analytics[template.uri].metrics.conversions?.toLocaleString()}
                            </Text>
                          </View>
                        )}
                      </View>
                      ...
                      <View className='mt-4'>
                        ...
                        {analytics[template.uri].performance.trends.map((trend, index) => (
                  ...
                  {optimizations[template.uri] && (
                    <View className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4'>
                      <Text className='font-semibold mb-2 dark:text-white'>Recommendations</Text>
                      {optimizations[template.uri].recommendations.map((rec, index) => (
                        <View key={index} className='mb-2'>
                          <View className='flex-row justify-between'>
                            <Text
                          ...
                          <Text className='text-gray-600 dark:text-gray-400'>{rec.suggestion}</Text>
                        </View>
                      ))}
                      ...
                      {optimizations[template.uri].abTests.length > 0 && (
                        <View className='mt-4'>
                          ...
                          {optimizations[template.uri].abTests.map((test, index) => (
                            <View key={index} className='mb-2'>
                              <Text className='text-gray-600 dark:text-gray-400'>
                                Variant {test.variant}: {(test.performance * 100).toFixed(1)}% (
                                {test.sampleSize} samples, {(test.confidence * 100).toFixed(1)}%
                                confidence)
                              </Text>
                            </View>
                          ))}
      ...
      {/* Schedules */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Content Schedules</Text>
        {schedules
          .filter((schedule) => schedule.name.toLowerCase().includes(searchQuery.toLowerCase()))
          .map((schedule) => (
            <View key={schedule.uri} className='mb-4 border-b border-gray-200 pb-4'>
              <View className='flex-row justify-between'>
                <Text className='font-semibold dark:text-white'>{schedule.name}</Text>
                <Text
                  className={`${
              ...
              {schedule.status === 'paused' && (
                ...
                >
                  <Text className='text-white text-center'>Resume Schedule</Text>
                </TouchableOpacity>
              )}
            </View>
          ))}
      ...
      >
        <Text className='text-white text-center font-bold'>Create New Template</Text>
      </TouchableOpacity>
    </ScrollView>
  );
};
