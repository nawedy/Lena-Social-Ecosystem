...
export const PaymentDashboard: React.FC = () => {
  ...
  const _loadData = async () => {
    try {
      const [methodsData, transactionsData, payoutAccountsData, analyticsData] = await Promise.all([
        // Get payment methods
  ...
  if (loading) {
    return (
      <View className='flex-1 justify-center items-center'>
        <Text className='text-lg dark:text-white'>Loading payment dashboard...</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView className='flex-1 bg-gray-100 dark:bg-gray-900'>
      ...
      {analytics && (
        <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
          <Text className='text-xl font-bold mb-4 dark:text-white'>Payment Analytics</Text>
          <View className='flex-row flex-wrap'>
            <View className='w-1/2 p-2'>
              <Text className='text-gray-600 dark:text-gray-400'>Total Volume</Text>
            ...
            <View className='w-1/2 p-2'>
              <Text className='text-gray-600 dark:text-gray-400'>Average Amount</Text>
              <Text className='text-2xl font-bold dark:text-white'>
                ${analytics.metrics.averageAmount.toLocaleString()}
              </Text>
            </View>
            <View className='w-1/2 p-2'>
              <Text className='text-gray-600 dark:text-gray-400'>Refund Rate</Text>
              <Text className='text-2xl font-bold dark:text-white'>
          ...
          <View className='mt-4'>
            ...
            {analytics.trends.map((trend, index) => (
              <Text key={index} className='text-gray-600 dark:text-gray-400'>
                ...
                {(trend.change * 100).toFixed(1)}%{trend.insight && ` - ${trend.insight}`}
              </Text>
            ))}
          </View>
        </View>
      ...
      {/* Payment Methods */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Payment Methods</Text>
        {paymentMethods.map((method) => (
          <View key={method.uri} className='mb-4 border-b border-gray-200 pb-4'>
            <View className='flex-row justify-between'>
              ...
              <Text className={getStatusColor(method.status)}>{method.status}</Text>
            </View>
            {method.lastFour && (
              <Text className='text-gray-600 dark:text-gray-400'>•••• {method.lastFour}</Text>
            )}
            {method.expiryDate && (
              <Text className='text-gray-600 dark:text-gray-400'>Expires: {method.expiryDate}</Text>
            )}
          </View>
        ))}
      ...
      {/* Recent Transactions */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Recent Transactions</Text>
        {transactions
          .filter(
            (transaction) =>
              transaction.uri.toLowerCase().includes(searchQuery.toLowerCase()) ||
          ...
          .map((transaction) => (
            <TouchableOpacity
              ...
              onPress={() =>
                setSelectedTransaction(
                  selectedTransaction === transaction.uri ? null : transaction.uri
                )
              }
            ...
            >
              <View className='flex-row justify-between'>
                ...
                <Text className={getStatusColor(transaction.status)}>{transaction.status}</Text>
              </View>
              ...
              {selectedTransaction === transaction.uri && (
                <View className='mt-4 bg-gray-50 dark:bg-gray-700 p-4 rounded-lg'>
                  ...
                  <Text className='text-gray-600 dark:text-gray-400'>To: {transaction.toDid}</Text>
                  {transaction.reference && (
                    <Text className='text-gray-600 dark:text-gray-400'>
                      Reference: {transaction.reference.type} ({transaction.reference.uri})
                    </Text>
                  )}
                  {transaction.metadata && (
                    <View className='mt-2'>
                      <Text className='font-semibold dark:text-white'>Metadata</Text>
                      {Object.entries(transaction.metadata).map(([key, value]) => (
                        <Text key={key} className='text-gray-600 dark:text-gray-400'>
                          {key}: {String(value)}
                        </Text>
                      ))}
                    </View>
      ...
      {/* Payout Accounts */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Payout Accounts</Text>
        {payoutAccounts.map((account) => (
          <View key={account.uri} className='mb-4 border-b border-gray-200 pb-4'>
            <View className='flex-row justify-between'>
              <Text className='font-semibold dark:text-white'>
                {account.type === 'bank_account' ? 'Bank Account' : 'Crypto Wallet'}
              </Text>
              <Text className={getStatusColor(account.status)}>{account.status}</Text>
            </View>

            {account.type === 'bank_account' && account.accountDetails.bankName && (
              <Text className='text-gray-600 dark:text-gray-400'>
                {account.accountDetails.bankName}
              </Text>
            )}

            {account.type === 'crypto_wallet' && account.accountDetails.walletAddress && (
              <Text className='text-gray-600 dark:text-gray-400'>
            ...
            {account.verification && account.verification.status !== 'verified' && (
              <View className='mt-2'>
                ...
                {account.verification.requiredDocs.length > 0 && (
                  <View>
                    <Text className='text-gray-600 dark:text-gray-400'>Required Documents:</Text>
