  ...
  if (loading || !supplier || !performance) {
    return (
      <View className='flex-1 justify-center items-center'>
        <Text className='text-lg dark:text-white'>Loading supplier dashboard...</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView className='flex-1 bg-gray-100 dark:bg-gray-900'>
      ...
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Performance Overview</Text>
        <View className='flex-row flex-wrap'>
          ...
          <View className='w-1/2 p-2'>
            ...
            <Text className='text-2xl font-bold dark:text-white'>{performance.orders}</Text>
          </View>
          <View className='w-1/2 p-2'>
            <Text className='text-gray-600 dark:text-gray-400'>Fulfillment Rate</Text>
            <Text className='text-2xl font-bold dark:text-white'>
              {(performance.fulfillmentRate * 100).toFixed(1)}%
            </Text>
          </View>
          <View className='w-1/2 p-2'>
            <Text className='text-gray-600 dark:text-gray-400'>Avg. Shipping Days</Text>
            <Text className='text-2xl font-bold dark:text-white'>
      ...
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Inventory Management</Text>
          ...
          <View key={center.id} className='mb-4'>
            ...
            <View className='space-y-2'>
              {Object.entries(center.inventory).map(([productId, quantity]) => (
                <View key={productId} className='flex-row justify-between items-center'>
                  <Text className='dark:text-white'>{productId}</Text>
                  <Text className={`${quantity < 10 ? 'text-red-500' : 'dark:text-white'}`}>
                    {quantity} units
                  </Text>
                </View>
              ))}
      ...
      {/* Recent Orders */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Recent Orders</Text>
