...
export const OrderTrackingDashboard: React.FC = () => {
  ...
  const _updateOrderStatus = async (orderUri: string, status: string, note?: string) => {
    try {
    ...
    <ScrollView className='flex-1 bg-gray-100 dark:bg-gray-900'>
      ...
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        ...
        {filteredOrders.map((order) => (
          <TouchableOpacity
            key={order.uri}
            onPress={() => setSelectedOrder(selectedOrder?.uri === order.uri ? null : order)}
            ...
            {selectedOrder?.uri === order.uri && (
              <View className='mt-4'>
                ...
                <View className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4'>
                  <Text className='font-semibold mb-2 dark:text-white'>Shipping Details</Text>
                  <Text className='text-gray-600 dark:text-gray-400'>{order.shipping.address}</Text>
                  <Text className='text-gray-600 dark:text-gray-400'>
                ...
                {order.tracking && (
                  <View className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4'>
                    <Text className='font-semibold mb-2 dark:text-white'>Tracking Info</Text>
                    <Text className='text-gray-600 dark:text-gray-400'>
                ...
                {/* Timeline */}
                <View className='mt-4'>
                  <Text className='font-semibold mb-2 dark:text-white'>Order Timeline</Text>
                ...
                <View className='flex-row flex-wrap mt-4'>
                  {order.status === 'pending' && (
                    ...
                    >
                      <Text className='text-white text-center'>Process Order</Text>
                    </TouchableOpacity>
                  )}
