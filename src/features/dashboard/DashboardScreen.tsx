...
import { ATProtocolDashboard, MultiAccountDashboard } from '../../services/atProtocolDashboard';

export const DashboardScreen: React.FC = () => {
  const { agent } = useATProto();
  const [dashboardData, setDashboardData] = useState<MultiAccountDashboard | null>(null);
  ...
  const _loadDashboardData = async () => {
    try {
      ...
      const _data = await dashboard.getMultiAccountDashboard([agent.session?.did || '']);
  ...
  return (
    <ScrollView className='flex-1 bg-gray-100 dark:bg-gray-900'>
      ...
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-lg font-semibold mb-4 dark:text-white'>Reach Trend</Text>
        <LineChart
          data={{
