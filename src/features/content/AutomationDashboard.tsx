  ...
  if (loading) {
    return (
      <View className='flex-1 justify-center items-center'>
        <Text className='text-lg dark:text-white'>Loading automation dashboard...</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView className='flex-1 bg-gray-100 dark:bg-gray-900'>
      ...
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Active Automations</Text>
        {automations.map((automation) => (
          <View key={automation.uri} className='mb-4 border-b border-gray-200 pb-4'>
            <View className='flex-row justify-between'>
              <Text className='font-semibold dark:text-white'>{automation.name}</Text>
              <Text
                className={`${
        ...
        <TouchableOpacity className='bg-blue-500 rounded-lg p-3 mt-2' onPress={createAutomation}>
          <Text className='text-white text-center font-semibold'>Create New Automation</Text>
        </TouchableOpacity>
      </View>

      {/* Scheduled Posts */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Scheduled Posts</Text>
        {scheduledPosts.map((post) => (
          <View key={post.uri} className='mb-4 border-b border-gray-200 pb-4'>
            ...
            <View className='flex-row justify-between mt-2'>
              <Text className='text-gray-600 dark:text-gray-400'>
                {post.content.media ? `${post.content.media.length} media items` : 'No media'}
              </Text>
              <Text
                className={`${
      ...
      {/* Trending Topics */}
      <View className='bg-white dark:bg-gray-800 m-4 p-4 rounded-lg'>
        <Text className='text-xl font-bold mb-4 dark:text-white'>Trending Topics</Text>
        {trends.map((trend) => (
          <View key={trend.topic} className='mb-4 border-b border-gray-200 pb-4'>
            <View className='flex-row justify-between'>
              <Text className='font-semibold dark:text-white'>{trend.topic}</Text>
              <Text className='text-blue-500'>Score: {trend.score.toFixed(2)}</Text>
            </View>
            ...
            {trend.suggestedContent.map((suggestion, index) => (
              <View key={index} className='mt-2'>
                ...
                <Text className='text-green-500'>
                  Est. Performance: {(suggestion.estimatedPerformance * 100).toFixed(1)}%
                </Text>
              </View>
            ))}
          </View>
